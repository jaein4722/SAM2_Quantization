...
class LayerBitController(nn.Module):
    ...
    def export_state(self) -> Dict[str, float]:
        return {name: float(val.detach().item()) for name, val in self.bits().items()}
...
class ActivationQuantizer(nn.Module):
    ...
    def _quantize_output(
        ...
        if isinstance(output, torch.Tensor):
            return self._quantize_tensor(output, bit_value, state, module_training, name)
        if isinstance(output, (tuple, list)):
            quantized = [
                self._quantize_output(item, bit_value, state, module_training, name)
        ...
        if isinstance(output, dict):
            return {
                key: self._quantize_output(val, bit_value, state, module_training, name)
    ...
    def _quantize_tensor(
        ...
        max_tensor = torch.as_tensor(max_val, dtype=bit_value.dtype, device=bit_value.device)
        qmax = torch.clamp((2 ** (bit_value - 1.0)) - 1.0, min=1.0)
        qmin = -torch.pow(2.0, bit_value - 1.0)
        scale = max_tensor / qmax
        scale = torch.clamp(scale, min=1e-6)
        with torch.no_grad():
            x_scaled = quant_input / scale
            clipped = (x_scaled < qmin) | (x_scaled > qmax)
            clip_rate = float(clipped.float().mean().item())
            self._stats[name] = {
                "scale": float(scale.detach().mean().item()),
                "zero_point": 0.0,
                "clip_rate": clip_rate,
                "max_val": float(max_val.detach().item()),
            }
