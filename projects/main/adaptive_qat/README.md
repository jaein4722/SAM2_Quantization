...
2. Optionally edit `paths.importance_json` to point to a custom JSON with recommended bitwidths/importance values (same schema as `baselines/layerwise_quantization/configs/ptq/bit_recommendation_v4_aggressive.json`).
3. Launch training:
   ```bash
   python projects/main/adaptive_qat/train_adaptive_qat.py --config projects/main/adaptive_qat/configs/adaptive_qat_template.yaml --experiment-dir <exp_dir>
   ```
