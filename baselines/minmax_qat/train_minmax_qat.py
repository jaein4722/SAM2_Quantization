...
from .utils import export_inference_checkpoint


DEFAULT_CONFIG = "baselines/minmax_qat/configs/minmax_toy.yaml"
...
def main() -> None:
    ...
    # Export a SAM2-compatible checkpoint for inference.
    checkpoint_path = Path(exp_dir) / "checkpoints" / "checkpoint.pt"
    if checkpoint_path.exists():
        export_path = export_inference_checkpoint(checkpoint_path)
        print(f"[minmax_qat] Exported inference checkpoint to {export_path}")
    else:
        print(f"[minmax_qat] WARNING: checkpoint not found at {checkpoint_path}; skipping export.")
